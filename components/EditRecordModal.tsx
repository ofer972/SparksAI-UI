...
export function EditRecordModal<T extends Record<string, any>>({
 ...
 >
 {fieldConfig.label}
 {fieldConfig.required && <span className="text-danger-text ml-1">*</span>}
 </label>
 <textarea
 id={fieldId}
 ...
 rows={12}
 className={`w-full px-3 py-2 border rounded-lg text-sm bg-surface text-content-primary focus:outline-none focus:ring-2 ${
 error ? 'border-danger-border focus:ring-danger-border' : 'border-outline-strong focus:ring-brand'
 ...
 <p className="mt-1 text-sm text-content-muted">{fieldConfig.helpText}</p>
 )}
 </div>
 );
 }
 ...
 <label
 htmlFor={fieldId}
 className="text-sm font-medium text-content-secondary w-40 flex-shrink-0 pt-2"
 >
 {fieldConfig.label}
 {fieldConfig.required && <span className="text-danger-text ml-1">*</span>}
 </label>
 ...
 value ? 'bg-brand' : 'bg-surface-elevated'
 }`}
 disabled={isDisabled}
 >
 <span
 ...
 <span className="text-sm text-content-tertiary">{value ? 'Yes' : 'No'}</span>
 </div>
 ) : (
 <input
 id={fieldId}
 ...
 <div
 ref={panelRef}
 className="bg-surface rounded-lg shadow-xl border border-outline max-w-[50.4rem] w-full mx-4 max-h-[95vh] overflow-hidden flex flex-col"
 ...
 {/* Footer */}
 <div className="flex justify-end gap-2 px-4 py-3 border-t border-outline bg-surface">
 <button
 type="button"
 onClick={onClose}
 className="px-4 py-2 text-sm font-medium text-content-secondary bg-surface-elevated border border-outline-strong rounded-lg hover:bg-surface-secondary transition-colors"
 >
 Cancel
 </button>
 <button
 type="submit"
 disabled={loading}
 className="px-4 py-2 text-sm font-medium text-white bg-brand rounded-lg hover:bg-brand-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
 >
 {loading ? 'Saving...' : 'Save'}
 </button>
 </div>
