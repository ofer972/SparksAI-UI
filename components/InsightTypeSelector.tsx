...
export default function InsightTypeSelector({
  ...
  const renderInsightTypeCard = (insightType: InsightType) => {
    ...
    return (
      <div
        key={insightType.id}
        className={`bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border-2 p-3 flex flex-col w-full ${
      ...
      >
        ...
        {insightType.description && (
          <p className="text-xs text-gray-600 mb-3 leading-relaxed ml-6">
            {insightType.description}
          </p>
        )}
        ...
        {(!insightType.requirePI && !insightType.requireTeam && !insightType.requireGroup) && isSelected && (
          <p className="text-xs text-gray-500 italic mb-2 ml-6">No filters required for this insight type.</p>
        )}
      </div>
    );
  };
