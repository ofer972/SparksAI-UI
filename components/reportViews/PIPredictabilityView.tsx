...
const PIPredictabilityView: React.FC<PIPredictabilityViewProps> = ({
 ...
 num >= 80 ? 'text-positive-text' : num >= 60 ? 'text-yellow-600 text-yellow-400' : 'text-danger-text'
 }`}
 >
 {num}%
 </span>
 ...
 className="w-56 px-2 py-1 border border-outline-strong bg-surface-elevated text-content-primary dark:placeholder:text-content-muted rounded text-xs focus:outline-none focus:ring-1 focus:ring-brand"
 />
 </ReportFilterField>
 </ReportFiltersRow>
 );
 ...
 {!loading && error && (
 <div className="flex items-center justify-center bg-danger-bg border border-danger-border rounded-lg p-4 text-sm text-danger-text h-64">
 {error}
 </div>
 )}
