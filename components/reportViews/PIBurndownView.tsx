...
const PIBurndownView: React.FC<PIBurndownViewProps> = ({
 ...
 // Custom filters for PI Burndown
 const customFilters = [
 {
 type: 'pi' as const,
 component: (
 ...
 value={issueType}
 disabled={true}
 className="px-2 py-1 border border-outline-strong rounded text-xs bg-surface-secondary cursor-not-allowed text-content-tertiary"
 >
 ...
 return badges;
 }, [piName, issueType, teamName, isGroup, pinnedFilters]);
 ...
 const fetchPIBurndownIssuesFactory = useCallback((selectedDate: string, selectedMetricType: string) => {
 return async () => {
 const pi = meta?.pi || piName;
 if (!pi) {
 ...
 }, [meta?.pi, piName, teamName, isGroup, issueType, apiService]);

 // Date display for PI
 const dateDisplay = Array.isArray(data) && data.length > 0 ? (
