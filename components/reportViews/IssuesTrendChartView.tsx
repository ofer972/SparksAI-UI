...
const IssuesTrendChartView: React.FC<IssuesTrendChartViewProps> = ({
 ...
 const canEditFilters = !!setFilters;
 ...
 return datasetLabel === 'Issues Left Open (Trend)' ? '#4169E1' : (isDark ? '#f1f5f9' : '#000');
 },
 font(context: any) {
 const datasetLabel = context.dataset.label || '';
 ...
 canEditFilters ? 'bg-surface' : 'bg-surface-secondary cursor-not-allowed'
 }`}
 >
 ...
 <div className="text-sm text-content-tertiary">Loading trend chart...</div>
 </div>
 </div>
 )}
 ...
 <Chart key={isDark ? 'dark' : 'light'} type="bar" data={chartData} options={options} plugins={[ChartDataLabels]} />
 </div>
 </div>
 )}
 </ReportCard>
