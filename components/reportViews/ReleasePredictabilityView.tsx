...
const buildColumns = (jiraUrl: string): Column<ReleasePredictabilityItem>[] => {
 ...
 if (value === 100) {
 return 'text-positive-text font-semibold';
 }
 ...
 type="button"
 onClick={handleClick}
 className="text-sm text-brand hover:text-blue-800 hover:text-blue-300 hover:underline"
 >
 {value}
 </button>
 );
 ...
 <div className="flex-1 bg-gray-200 bg-surface-elevated rounded-full h-3 overflow-hidden">
 <div
 className="h-full bg-green-500 transition-all duration-300"
 style={{ width: `${Math.min(progressPercent, 100)}%` }}
 />
 </div>
 <span className="text-sm font-medium text-content-primary min-w-[45px] text-right">{formattedPercent}%</span>
 </div>
 );
 },
 },
 ...
 return <span className="text-sm text-content-tertiary">-</span>;
 }
 return (
...
const ReleasePredictabilityView: React.FC<ReleasePredictabilityViewProps> = ({
 ...
 <span className="text-sm text-content-secondary">{rows.length}</span>
 </ReportFilterField>
 </ReportFiltersRow>
 );
