...
const HierarchyTable: React.FC<HierarchyTableProps> = ({
 ...
 <span className="inline-flex items-center justify-center w-5 h-5 rounded border-2 border-blue-600 border-blue-400 bg-brand/10">
 ...
 return (
 <div
 className={`${fontSizeClass} text-content-secondary`}
 style={{ paddingLeft: `${level * 20}px` }}
 >
 {displayValue}
 </div>
 ...
 className="text-xs text-content-tertiary hover:text-content-primary hover:text-content-secondary flex items-center justify-center w-6 h-6"
 ...
 placeholder="Search..."
 className="px-2 py-1.5 border border-outline-strong bg-surface-elevated text-content-primary rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-brand"
 />
 <button
 type="button"
 onClick={toggleAllExpanded}
 className="px-2 py-1 text-xs text-content-tertiary border border-outline rounded hover:bg-surface-elevated"
 >
 {Object.keys(expanded).length === 0 ? 'Expand all' : 'Collapse all'}
 </button>
 </div>
 <div className="text-sm text-content-muted">Rows: {flatData.length}</div>
 </div>
 )}

 <div className="flex-1 overflow-auto border border-outline rounded-lg max-h-[600px]">
 <table className="min-w-full divide-y divide-gray-200 divide-outline text-[13px]" style={{ tableLayout: 'fixed' }}>
 ...
 <th
 key={header.id}
 className="px-3 py-2 text-left text-[13px] font-medium text-content-tertiary text-content-tertiary uppercase tracking-wider bg-surface-elevated border-b border-r border-outline"
 style={headerStyle}
 >
 ...
 <tbody className="bg-surface divide-y divide-gray-200 divide-outline">
