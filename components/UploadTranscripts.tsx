...
const FileUploadArea = React.memo(({ 
 ...
 {/* File Selection Area */}
 <div
 className="border-2 border-dashed border-outline-strong rounded-lg p-3 text-center hover:border-outline-strong hover:border-outline-strong transition-colors cursor-pointer mt-2"
 ...
 <p className="text-xs text-content-muted mt-1">Maximum file size: 2MB</p>
 </div>
 <input
 ref={fileInputRef}
 type="file"
 ...
 {files.length > 0 && (
 <div className="mt-2">
 <h4 className="text-sm font-medium text-content-secondary mb-1">Selected Files:</h4>
 <div className="space-y-1">
 {files.map((fileWithId) => (
 <div key={fileWithId.id} className="flex items-center justify-between bg-surface-elevated p-2 rounded border border-outline">
 <span className="text-sm text-content-secondary">{fileWithId.file.name}</span>
 <button
 onClick={() => onRemoveFile(fileWithId.id)}
 className="text-danger-text hover:text-red-700 hover:text-red-300 text-sm"
 disabled={uploading}
 >
 Remove
 </button>
 ...
 {uploading && (
 <div className="mt-2">
 <div className="flex justify-between text-sm text-content-tertiary mb-1">
 <span>Uploading...</span>
 <span>{Math.round(progress)}%</span>
 </div>
 <div className="w-full bg-gray-200 bg-surface-secondary rounded-full h-2">
 <div
 className="bg-brand dark:bg-brand h-2 rounded-full transition-all duration-300"
 ...
 {/* Error Message */}
 {error && (
 <div className="mt-2 p-2 bg-danger-bg border border-danger-border rounded-md">
 ...
 {/* Success Message */}
 {success && (
 <div className="mt-2 p-2 bg-positive-bg border border-positive-border rounded-md">
 <p className="text-sm text-positive-text">{success}</p>
 </div>
 )}
 ...
 className="mt-2 w-full px-4 py-2 bg-brand text-white rounded-lg hover:bg-brand-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
 >
