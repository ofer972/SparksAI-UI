...
const DaysLeftCard = ({ id, daysLeft, daysInSprint, tooltip, className ="", activeTooltip, setActiveTooltip }: {
 ...
 return (
 <div 
 className={`bg-gradient-to-br from-surface to-surface-elevated rounded-lg border border-outline shadow-sm hover:shadow-md hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-200 p-2 sm:p-3 flex flex-col items-center justify-center text-center h-full relative min-w-0 ${className}`}
 ...
 <div className="text-base sm:text-lg font-bold text-content-primary mb-0.5 sm:mb-1">
 {formatDaysLeft(daysLeft)}
 </div>
 
 {/* Days Left Label */}
 <div className="text-[10px] sm:text-xs text-content-secondary font-semibold leading-tight">
 days left in Sprint
 </div>
 
 {/* Tooltip */}
 <div 
 className={`absolute bottom-full mb-2 px-3 py-2 bg-gray-800 text-white text-xs rounded transition-opacity duration-200 pointer-events-none z-50 max-w-xs left-1/2 transform -translate-x-1/2 ${isTooltipVisible ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
 style={{ 
 whiteSpace: 'normal',
...
const MetricCard = ({ id, icon, value, label, tooltip, className ="", isLeftmost = false, status, trendData, metricType, activeTooltip, setActiveTooltip }: { 
 ...
 <div className="text-[10px] sm:text-xs text-content-secondary break-words text-center font-semibold leading-tight">
 {label}
 </div>
 {/* Tooltip */}
 <div 
 className={`absolute bottom-full mb-2 px-3 py-2 bg-gray-800 text-white text-xs rounded transition-opacity duration-200 pointer-events-none z-50 max-w-xs ${isLeftmost ? 'left-0' : 'left-1/2 transform -translate-x-1/2'} ${isTooltipVisible ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
 style={{ 
 whiteSpace: 'normal',
 wordBreak: 'break-word',
 minWidth: '200px',
...
export default function TeamMetrics({ teamName, isGroup, selectedMetrics, singleRowLayout = false, refreshKey }: TeamMetricsProps) {
 ...
 <div 
 key={i} 
 className="bg-gradient-to-br from-surface to-surface-elevated rounded-lg border border-outline shadow-sm p-2 sm:p-3 animate-pulse h-full min-w-0"
 style={isLastItem ? { gridColumn: '1 / -1' } : {}}
 >
 <div className="w-6 h-6 sm:w-8 sm:h-8 bg-transparent rounded mb-1 sm:mb-2 mx-auto"></div>
 <div className="h-4 sm:h-5 bg-gray-200 bg-surface-elevated rounded mb-1 sm:mb-1.5"></div>
 <div className="h-2 sm:h-3 bg-gray-200 bg-surface-elevated rounded"></div>
 </div>
 );
 })}
 </div>
