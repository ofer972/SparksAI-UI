...
const DashboardLayoutPreview: React.FC<{ 
  ...
  if (!layoutConfig?.layoutConfig?.rows || layoutConfig.layoutConfig.rows.length === 0) {
    return (
      <div className="h-28 bg-gray-50 rounded-lg flex items-center justify-center border border-gray-200">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-400 font-medium">Empty dashboard</p>
        </div>
      </div>
    );
  }
  ...
  const renderWidgetPreview = (widget: DashboardWidget) => {
    ...
    return (
      <div key={widget.id} className="flex-1 min-w-0">
        <div className="bg-white rounded border border-gray-300 shadow-sm overflow-hidden h-full">
          ...
          <div className={`px-2 py-1 border-b border-gray-200 ${
            ...
            <div className="flex items-center gap-1">
              <div className="h-2 w-2 rounded bg-blue-400 flex-shrink-0"></div>
              <div className="text-[8px] font-semibold text-gray-700 truncate leading-tight">
                {widgetName}
              </div>
            </div>
          </div>
  ...
  return (
    <div className="relative h-28 bg-gray-50 rounded-lg border border-gray-200 p-1.5 overflow-hidden">
      ...
      {totalWidgets > maxPreviewWidgets && (
        <div className="absolute bottom-1.5 right-1.5 bg-gray-900/80 text-white text-[8px] font-semibold px-1.5 py-0.5 rounded shadow-sm z-10">
          +{totalWidgets - maxPreviewWidgets}
        </div>
      )}
    </div>
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[600px]">
        <div className="flex flex-col items-center space-y-3">
          ...
          <div className="text-sm text-gray-600">Loading dashboards...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col bg-white">
      <div className="flex-1 overflow-auto">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-900 placeholder-gray-400 transition-all"
                    disabled={isCreating}
                  />
                </div>
                <div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-10">
          {dashboards.length === 0 ? (
            <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl border-2 border-dashed border-gray-300 p-20 text-center">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-600 mb-8">Create your first custom dashboard to organize your reports and insights</p>
              </div>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 mb-1">Created Dashboards</h2>
                  <p className="text-gray-500 text-sm">{dashboards.length} {dashboards.length === 1 ? 'dashboard' : 'dashboards'}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {dashboards.map((dashboard) => (
                  <div
                    key={dashboard.id}
                    className="group bg-white rounded-2xl border-2 border-gray-200 hover:border-blue-400 hover:shadow-2xl transition-all duration-300 overflow-hidden flex flex-col transform hover:-translate-y-1"
                  >
                    {/* Card Header with Gradient */}
                    <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 px-6 pt-6 pb-4 border-b border-gray-100">
                      <div className="flex items-start justify-between gap-3 mb-4">
                        <div className="flex-1 min-w-0">
                          ...
                          {dashboard.description && (
                            <p className="text-sm text-gray-600 line-clamp-2">{dashboard.description}</p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="px-6 py-5 bg-white mt-auto">
                      <div className="flex gap-3">
                        <button
                          ...
                          className="flex-1 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all text-sm font-bold flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
                        >
                          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <button
                          ...
                          className="px-4 py-2.5 bg-white border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-red-50 hover:border-red-400 hover:text-red-600 transition-all text-sm flex items-center justify-center shadow-sm hover:shadow-md"
