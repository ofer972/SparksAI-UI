...
export default function SprintInsightsPreview({
  teamOrGroupName,
  isGroup,
  onOpenCard,
  onOpenAll,
}: Props) {
  // Use Sprint Events and Sprint Status categories - memoized to prevent infinite loops
  const categories = useMemo(() => ['Sprint Events', 'Sprint Status'], []);
  const { cards, loading, error } = useAICards(undefined, teamOrGroupName, categories, isGroup);
      ...
      <div className="px-4 py-3 border-b border-outline bg-gradient-to-r from-surface to-surface-elevated flex-shrink-0">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm font-semibold text-content-primary">Sprint Insights</div>
            <div className="text-xs text-content-tertiary">
              Sprint Events & Status insights
            </div>
          </div>
          <button
            type="button"
      ...
      <div className="p-4 flex-1 flex flex-col">
        {!teamOrGroupName ? (
          <div className="text-sm text-content-tertiary">
            Set a default team/group to see sprint insights.
          </div>
        ) : loading ? (
          ...
          <div className="text-sm text-content-tertiary">No sprint insights available yet.</div>
        ) : (
